...
public class TarefasController : ControllerBase
{
    ...
    [HttpGet("do-dia/setor/{setorId:int}")] 
...
// POST: api/tarefas
[HttpPost]
public async Task<IActionResult> CreateTarefa([FromBody] TarefaDto.TarefaCreateRequest dto)
{
    _logger.LogInformation("Recebida requisição para criar nova tarefa.");
    ...
    try
    {
        ...
        var tarefa = await _tarefaService.CreateTarefaAsync(dto, criadorId);
        return CreatedAtRoute("GetTarefaById", new { id = tarefa.Id }, tarefa);
    }
    ...
    catch (Exception ex)
    {
        ...
        return StatusCode(500, new { mensagem = "Erro interno ao criar a tarefa.", detalhe = ex.Message });
    }
}

// PUT: api/tarefas/{id}
[HttpPut("{id:int}")]
public async Task<IActionResult> AtualizarTarefa(int id, [FromBody] TarefaDto.TarefaUpdateRequest dto)
{
    _logger.LogInformation("Recebida requisição para atualizar tarefa {Id}.", id);
    ...
    {
        return NotFound(new { mensagem = $"Tarefa com ID {id} não encontrada para atualização." });
    }
    catch (ArgumentException ex)
    {
        _logger.LogWarning(ex, "Erro de validação ao atualizar tarefa.");
        return BadRequest(new { mensagem = ex.Message });
    }
    catch (Exception ex)
    {
        _logger.LogError(ex, "Erro interno ao atualizar tarefa.");
        return StatusCode(500, new { mensagem = "Erro interno ao atualizar a tarefa.", detalhe = ex.Message });
    }
}
...
{
    _logger.LogInformation("Recebida requisição para cancelar tarefa {Id}.", id);

    try
    {
        await _tarefaService.CancelarTarefaAsync(id);
        return NoContent();
    }
    catch (KeyNotFoundException)
    {
        return NotFound(new { mensagem = $"Tarefa com ID {id} não encontrada para cancelamento." });
    }
    catch (Exception ex)
    {
        _logger.LogError(ex, "Erro interno ao cancelar tarefa.");
        return StatusCode(500, new { mensagem = "Erro interno ao cancelar a tarefa.", detalhe = ex.Message });
    }
}
}
